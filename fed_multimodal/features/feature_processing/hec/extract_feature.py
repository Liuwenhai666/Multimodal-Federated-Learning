...
if __name__ == '__main__':
    ...
    # 将分组后的数据格式转换成 NumPy
    data_by_turbid = {
        turbid: group.drop(columns=["TurbID"])    # 删除TurbID
    ...
    for turbid, data in tqdm(data_by_turbid.items()):
        ...
        data_by_turbid[turbid][['f1','f2','f3','t1','t2','t3','label']] = scaler.fit_transform(data_by_turbid[turbid][['f1','f2','f3','t1','t2','t3','label']])
        data_by_turbid[turbid] = data_by_turbid[turbid][['f1','f2','f3','t1','t2','t3','label']].reset_index(drop=True)
            
        model1_dict[turbid] = data_by_turbid[turbid][['f1','f2','f3','label']].to_numpy()
        model2_dict[turbid] = data_by_turbid[turbid][['t1','t2','t3','label']].to_numpy()
